...
export default function App() {
  ...
  const handleUnlock = () => {
    ...
    // Log email (replace with real endpoint when ready)
    console.log("[NatSel.ai] Email captured:", unlockEmail);
    try {
      fetch("/api/capture-email", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: unlockEmail, jobTitle, industry, rating }),
      }).catch(() => {}); // silently fail if endpoint doesn't exist yet
    } catch (_) {}

    // Save plan data to localStorage for /success PDF download
    const userPlanData = {
      email: unlockEmail,
      jobTitle,
      industry,
    ...
    // Show full plan in-browser immediately
    ...
    <div style={{ background: "#121212", minHeight: "100vh" }}>
      ...
      >
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: "900px" }}>
          ...
          >
            ...
            years out — and how to get ahead and <span style={{ color: "#D2042D" }}>harness AI tools before it's too late</span>.
          </p>
      ...
      {showResults && (
        ...
        >
          ...
          {/* ── Unlock Career Survival Kit ── */}
          {showPaywall && !planUnlocked && (
            <div
              id="unlock"
              style={{
                textAlign: "center",
            ...
            >
              ...
              <h3 style={{ fontSize: "24px", fontWeight: 800, margin: "12px 0 8px" }}>
                ROCA Method: ROCA through DROCASTLE-AI
              </h3>
              <p
                style={{
                  color: "#aaa",
              ...
              >
                ...
                <span style={featureTag}>🧠 ROCA → DROCASTLE-AI</span>
              </div>
              {/* Email input */}
              ...
              <button
                ...
                disabled={!isValidEmail(unlockEmail)}
                style={{
                  padding: "16px 48px",
                  background: isValidEmail(unlockEmail) ? "#D2042D" : "#555",
                  color: "#F0F0F0",
                  border: "none",
                  borderRadius: "10px",
                  fontSize: "18px",
                  fontWeight: 800,
                  cursor: isValidEmail(unlockEmail) ? "pointer" : "not-allowed",
                  boxShadow: isValidEmail(unlockEmail) ? "0 0 25px rgba(210,4,45,0.3)" : "none",
                  transition: "all 0.2s",
                }}
              >
                Unlock Career Survival Kit
              ...
              <p style={{ color: "#555", fontSize: "12px", marginTop: "12px" }}>
                100% free · Instant access · PDF download available
              </p>
            </div>
          )}
          ...
          {planUnlocked && (
            ...
            >
              ...
              <h3 style={{ color: "#5aff5a", fontSize: "22px", fontWeight: 800, margin: "0 0 8px" }}>
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    const { generatePDF } = await import("./pages/Success.jsx");
                    ...
                    const name = (jobTitle || "Career").replace(/[^a-zA-Z0-9]/g, "_");
              ...
              <p style={{ color: "#555", fontSize: "12px", marginTop: "16px" }}>
                PDF generates instantly in your browser
              </p>
            </div>
          )}
      ...
      <footer style={{ borderTop: "1px solid #1e1e1e", padding: "32px 20px", textAlign: "center" }}>
        ...
        <p style={{ color: "#333", fontSize: "12px", marginTop: "4px" }}>
          © 2026 Natsel.ai — All rights reserved.
        </p>
      </footer>
    </div>
  );
